...
const RATE_LIMIT_REQUESTS = 20; // Máximo 20 requests por ventana
...
function checkRateLimit(clientId: string): { allowed: boolean; remaining: number; resetIn: number } {
  ...
  // Limpiar registros expirados periódicamente (cada 100 llamadas)
  if (Math.random() < 0.01) {
...
function getClientId(req: Request): string {
  // Usar IP del cliente o fallback al header x-forwarded-for
